...
export function getServerVersion(context: ExtensionContext): string {
  const version: string = context.globalState.get("server_version") ?? "0.0.0"; // provide default value
...
export function getParserName(): string {
  switch (getPlatform()) {
    case Platform.Windows:
      return "parser-win.aot";
    case Platform.Linux:
      return "parser-linux.aot";
    case Platform.Mac:
      return "parser-mac.aot";
  }
}
